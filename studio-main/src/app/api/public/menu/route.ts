...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('SERVICE_API_KEY length:', SERVICE_API_KEY?.length);
    ...
    const headers: Record<string, string> = {
      ...
      'X-API-Key': SERVICE_API_KEY, // Force sending the key
    };
    ...
    const transformedData = Array.isArray(data) ? data.map((item: any) => {
      ...
      if (imageUrl) {
        if (!imageUrl.startsWith('http')) {
          ...
          if (imageUrl.startsWith('/static/')) imageUrl = `${serverBase}${imageUrl}`;
          else imageUrl = `${serverBase}/${imageUrl}`;
        }
      } else {
